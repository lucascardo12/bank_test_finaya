...
Retorna o saldo da carteira em um instante específico do tempo. Aceita `LocalDateTime` como parâmetro no formato
ISO-8601.
...
2. **Inicie o PostgreSQL com Docker Compose:**
   ...
   **Nota:** As tabelas serão criadas automaticamente pelo Hibernate na primeira execução. Se preferir criar
   manualmente, veja a seção "Criar Tabelas Manualmente" na Opção 2.
...
4. **Crie as tabelas manualmente (opcional):**
   ...
   Por padrão, o Hibernate cria as tabelas automaticamente (`spring.jpa.hibernate.ddl-auto=update`). Se preferir criar
...
### Fluxo de Teste: Transferência PIX

### Exemplo Prático de Teste Manual

Siga estes passos para testar uma transferência PIX manualmente:
...
#### **Passo 4: Iniciar Transferência PIX**
...
#### **Passo 5: Confirmar Transferência via Webhook**
...
**Compromisso:** Para MVP/protótipo, banco relacional é suficiente. Em produção com alto volume, considerar Redis para
...
**Compromisso:** Para volumes moderados, a solução atual é adequada. Em produção com alto volume, implementar snapshots
periódicos ou materialized views.
...
**Compromisso:** Para o escopo atual, síncrono é suficiente. Em produção, considerar fila de mensagens (RabbitMQ/Kafka)
...
**Compromisso:** Trade-off aceito pela clareza e testabilidade. Em projetos maiores, considerar injeção automática com
`@Component` e `@Service`.
